...
- 2026-08-09: Preflight passed. `API_KEYS` had the signing secret under the typo `SLACK_SIGNING_SECERT`; it now exposes canonical `SLACK_SIGNING_SECRET` without duplicating the secret. Slack verification uses the checked-in manifest plus bot `auth.test`, since `slack app list` requires a Slack project directory.
